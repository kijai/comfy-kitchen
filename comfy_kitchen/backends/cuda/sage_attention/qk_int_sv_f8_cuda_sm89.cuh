...
#pragma unroll
  ...
  // smem_V.get_permuted_offset(get_warp_idx_k<num_warps_q, num_warps_k>() *
  // WARP_K + lane_id % 16, lane_id / 16);
  uint32_t V_smem_offset_mma = smem_V.get_permuted_offset(
      lane_id % 8 + (lane_id / 16) * 8,
      get_warp_idx_k<num_warps_q, num_warps_k>() * WARP_K / PACK_SIZE_V +
          (lane_id / 8) % 2);
...
#pragma unroll
  ...
  // load V
  // ! we assume that V is padded. If not, there might be illegal memory access
  // or nan issue. for fp16: load_global_to_share                stride_seq_v
  load_fp8_V_global_to_share<global_to_shared_line_lanes_V,
...
#pragma unroll
    ...
    // for fp16:
    // compute_fp16_sv_permuted<num_warps_q, num_warps_k, num_tiles_q,
    // num_tiles_k, num_tiles_v, swizzle_mode_V, V_SMEM_STRIDE / PACK_SIZE_V,
    // 4>(
    //   smem_V, RS_f16, RO, d, V_smem_offset_mma);
    if constexpr (!use_inst_buffer) {
      compute_fp8_sv<num_warps_q, num_warps_k, num_tiles_q, num_tiles_k,
                     num_tiles_v, swizzle_mode_V, V_SMEM_STRIDE / PACK_SIZE_V>(
          smem_V, RS_f8, RO, d);
    } else {
      if constexpr (!use_pv_fp16_accu) {
        compute_fp8_sv_inst_buf<num_warps_q, num_warps_k, num_tiles_q,
                                num_tiles_k, num_tiles_v, swizzle_mode_V,
                                V_SMEM_STRIDE / PACK_SIZE_V>(smem_V, RS_f8, RO,
                                                             d);
      } else {
        compute_fp8_sv_inst_buf_fp16_accu<
            num_warps_q, num_warps_k, num_tiles_q, num_tiles_k, num_tiles_v,
            swizzle_mode_V, V_SMEM_STRIDE / PACK_SIZE_V>(smem_V, RS_f8, RO, d);
      }
    }
    __syncthreads();
    // load V
...
#pragma unroll
    ...
    if constexpr (mask_mode == MaskMode::kCausal) {
      apply_causal_mask<num_tiles_q, num_tiles_k>(Q_idx_lane_base,
                                                  K_idx_lane_base, RS_f32);
    }
    // apply_out_of_bound_mask<num_tiles_q, num_tiles_k>(K_idx_lane_base,
    // RS_f32, kv_len);
    K_idx_lane_base += CTA_K;

    if constexpr (std::is_same<DTypeSVAccum, float>::value) {
      update_mdo<num_tiles_q, num_tiles_k, num_tiles_v, false, true, false>(
          RS_f32, RO, m, d, original_sm_scale);
    } else if constexpr (std::is_same<DTypeSVAccum, half>::value) {
      update_mdo<num_tiles_q, num_tiles_k, num_tiles_v, true, true, false>(
          RS_f32, RO, m, d, original_sm_scale);
    }
...
#pragma unroll
    ...
    apply_out_of_bound_mask<num_tiles_q, num_tiles_k>(K_idx_lane_base, RS_f32,
                                                      kv_len);
    K_idx_lane_base += CTA_K;
